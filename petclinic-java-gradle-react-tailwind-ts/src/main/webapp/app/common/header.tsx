    ...
    <header ref={headerRef} className="bg-gray-50">
      <div className="container mx-auto px-4 md:px-6">
        <nav className="flex flex-wrap items-center justify-between py-2">
          ...
          <button
            type="button"
            className="js-dropdown md:hidden border rounded cursor-pointer"
          ...
          <div
            className="hidden md:block flex grow md:grow-0 justify-end basis-full md:basis-auto pt-3 md:pt-1 pb-1"
            id="navbarToggle"
          >
            <ul className="flex">
              <li>
                <Link to="/" className="block text-gray-500 p-2">
                  {t('navigation.home')}
                </Link>
              </li>
              <li>
                {authenticationContext.isLoggedIn() ? (
                  <button
                    ...
                    className="block text-gray-500 p-2 cursor-pointer"
                  >
                    {t('navigation.logout')}
                  </button>
                ) : (
                  <Link to="/login" className="block text-gray-500 p-2">
                    {t('navigation.login')}
                  </Link>
                )}
              </li>
